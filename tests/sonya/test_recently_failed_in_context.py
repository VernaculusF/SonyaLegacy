...
Root cause: TaskStore.list_open() filters status IN
...
Fix: TaskStore.list_recently_failed(hours=6) and a small block in
...
def test_list_recently_failed_returns_recent(substrate: Substrate) -> None:
    ...
    assert found[0].status is TaskStatus.FAILED


def test_list_recently_failed_skips_old(substrate: Substrate) -> None:
    """Tasks failed >6h ago do NOT surface."""
    svc = TaskService(TaskStore(substrate), stream=ContinuityStream(substrate))
    ...
    found = TaskStore(substrate).list_recently_failed(hours=6)
    assert found == []
...
def test_list_recently_failed_limit(substrate: Substrate) -> None:
    ...
    found = TaskStore(substrate).list_recently_failed(hours=6, limit=2)
    assert len(found) == 2
