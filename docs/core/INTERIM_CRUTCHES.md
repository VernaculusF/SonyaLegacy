...
### CRUTCH-015: Pre-DONE self-critique как второй LLM-вызов

**Что происходит:** После каждого TG-ответа запускается **отдельный** LLM call (purpose=`pre_done_critique`) который проверяет гендер, эмодзи, prompt-echo и правит текст перед отправкой.

**Почему это костыль:**
- Это **второй** inference на каждый ответ — +2-3 сек latency, +200-400 tokens/ответ.
- Настоящая модель не "проверяет себя снаружи" — она просто не делает ошибок гендера. Это workaround потому что Kimi K2.6 reasoning-модель которая леет мысли.
- Self-critique дублирует то что промпт уже говорит ("используй женский род") — но модель игнорирует промпт и делает ошибки всё равно.

**Что будет вместо:** На RWKV с State Tuning гендер зашит в state. Модель стартует как женщина — не нужно ни проверять ни напоминать. Prompt-echo невозможен потому что rules в state, не в видимом тексте.

**Когда уйдёт:** Stage 6 (RWKV migration). Или раньше если сменим на модель которая не леет reasoning.
