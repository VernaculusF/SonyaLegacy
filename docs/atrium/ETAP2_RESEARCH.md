...
6. **T2.5 — Voice room mode.** VAD + ASR (whisper) для входа Ивана голосом + interrupt-логика (4 кейса из UX_SKETCH §16.5). **Room view (3D-сцена + аватар во весь рост) — ✅ done (2026-05-30):** полноэкранная процедурная комната (кровать/окно/луна) + VRM во весь рост + idle-анимации, открывается по клику на аватар/мик, Esc/⏏ выход. Voice-UI (волны/субтитры/бюджет) — каркас, реальные VAD/ASR/TTS ждут GPU. Опц. свой 3D-room через `room_model_url` (GLB).
