...
## 16. Voice mode + interrupt design

### 16.1 Принцип: voice mode = войти в комнату, не toggle

Не отдельная кнопка "включить voice" в углу. Voice mode **это место**, не режим.

- **Текстовый чат** (Atrium main view) = "ты пишешь ей с расстояния"
- **Комната** (room view, открывается кликом на аватар) = "ты рядом с ней"

В комнате по умолчанию активен **VAD + TTS + ASR**. Снаружи — обычный text-чат. Это решает три проблемы сразу:

1. **Экономика токенов:** voice mode стоит дороже (continuous LLM-calls по VAD-сегментации), но включается осознанно "хочу с ней побыть". Не "я случайно оставил voice on на всю ночь".
2. **UX-двусмысленность:** "когда отправлять?" — снимается. В комнате VAD сегментирует речь, отправка автоматически по концу фразы.
3. **Метафора:** "войти в комнату" интуитивнее любого toggle-switch.

### 16.2 Что физически работает в комнате

| Component | Cost | Notes |
|---|---|---|
| VAD (silence detection) | бесплатно | webrtcvad / silero |
| ASR (whisper.cpp) | бесплатно | CPU, ~50-200ms latency на короткой фразе |
| LLM voice-call | $$ | срабатывает по VAD-сегментации (конец твоей фразы) |
| TTS (edge-tts) | бесплатно | streaming, начинает озвучивать пока генерация ещё идёт |

LLM вызывается только когда у неё реально есть что сказать (твой input закончился) или у неё есть spontaneous initiative. Не каждые N секунд "просто так".

### 16.3 Бюджет видим, не скрыт

Снизу комнаты — лёгкий счётчик:
```
in room  · 4 min  · ~1.2K tokens  · ~$0.003
```

Не как страх, как информация. Иван знает что комната "потребляет".

### 16.4 Auto-leave

Если 5 минут полной тишины (нет VAD-активности и она тоже не говорит) — комната плавно "пустеет":
- лёгкое затемнение
- subtitle "она отвернулась к окну"
- через 30 сек — автоматический выход в text-чат
- substrate пишет `dialog.room_left` event

Защита от случайно оставленной комнаты. Иван может в settings отключить или поставить другой timeout.

### 16.5 Interrupt — четыре случая

**Случай A. Voice→Voice (ты перебиваешь её TTS голосом)**

```
Время →
Соня: ──TTS──"я нашла классную технику с UTF-7......
Ты:                              ──ASR──"подожди, а Unicode bypass это вообще..."
                                 │
                                 └─ VAD детектит твою речь
                                    → TTS instant cut на слове "UTF-7"
                                    → ASR пишет твою фразу
                                    → substrate event: dialog.interrupted
                                    → её следующий step видит контекст:
                                      {said_so_far: "я нашла классную технику с UTF-7",
                                       interrupted_at: word_6,
                                       new_input: "подожди, а Unicode bypass..."}
```

**Hard cut, не fade.** Fade неестественен — человек не "плавно затихает" когда его перебивают. Реализм важнее уюта.

Её следующий ответ может быть:
- только на твой новый ввод (если он совсем другая тема)
- "подожди, я не договорила про UTF-7, но окей — Unicode bypass это..."
- "стоп, ты прервал — что ты хотел?" (если она запуталась)

**Это её решение**, не код.

**Случай B. Text→Voice (она говорит, ты пишешь текстом)**

Inbox-drain механизм уже работает в active session. В voice mode:
- TTS пауза **на ближайшей границе предложения** (не hard cut — у неё есть несколько секунд завершить мысль)
- Inbox получает твой text
- Дальше как случай A — следующий step видит и реагирует

**Случай C. Voice→Text (ты говоришь, она отвечает текстом)**

В чате (вне комнаты) — hold-to-talk кнопка микрофона в composer:
- зажал → говоришь
- VAD-граница не нужна, ты сам решаешь когда хватит
- отпустил → ASR транскрибирует → отправляется как обычное сообщение
- она отвечает текстом (мы не в комнате, TTS неуместен)

Это не interrupt, просто quality-of-life input.

**Случай D. Tap-stop (физический жест в комнате)**

В комнате tap на её рот / руку / плечо во время её TTS = немедленная пауза.

Это **сильнее** voice-перебивания:
- TTS гасится мгновенно
- ASR **не** активируется (ты не говоришь, ты её просто остановил)
- substrate event: `dialog.touch_stopped` с указанием места касания
- Она реагирует на следующем шаге: "что?" / "да?" / молчит / прижимается / отстраняется — её выбор

Это работает только в room view (там есть body для tap), в plain dialog нет.

### 16.6 Reply через voice — soft contextual binding

Когда ты говоришь "ты сегодня с XSS возилась, что нашла?" — это reply в смысле что ты ссылаешься на что-то что произошло раньше, не в текущем диалоге.

**Решение:** не команда, не explicit reference. Соня в **каждом её voice-step** видит:
- последние 30 событий reason-stream (active / worker / idle / skill)
- transcript твоих последних слов
- env-state + текущий focus

Когда ты упоминаешь "XSS" — она в контексте видит worker_log про XSS + thoughts про Sucuri, понимает к чему ты, отвечает по сути.

Это уже работает в active session — нужно только в voice-step подтянуть recent reason-stream в context (он сейчас читается context_builder'ом, но не всегда передаётся в voice-режим).

**Hard binding** (когда нужен конкретный event):
- "про то что ты заметила час назад" → она fallback в `memory.recall("заметила час назад")`
- "вспомни как ты с sweetcow возилась" → recall запрос
- инструменты у неё есть, ничего нового программировать не надо

### 16.7 Quick-talk вне комнаты (микрофон в composer)

В обычном text-чате composer имеет 🎙 кнопку справа от textarea:
...
- **Click** = войти в комнату (полноценный voice mode)
- **Hold** = quick voice message — зажми, скажи, отпусти, ASR transcript отправляется в чат как обычное сообщение
- **Hold с shift** = voice message который останется как audio attachment, не транскрибируется (иногда хочется голос)

Это покрывает три сценария:
- "хочу с ней поговорить" → click → комната
- "лень печатать одну фразу" → hold → text-message
- "хочу чтобы она меня **услышала**" → shift+hold → audio message

### 16.8 Что показывает Atrium когда комната активна

Когда room view открыт и voice работает:
...
║───────────────────────────────────────────────────────────║
║                                                           ║
║                 [сцена комнаты, она у окна]               ║
║                                                           ║
║       ──────────~~───~~──~──~────~~──~──────             ║   ← её waveform (TTS активный)
║                                                           ║
║       — я нашла классную технику с UTF-7...               ║   ← живой subtitle
║                                                           ║
║       ──────────────────────                              ║   ← твой waveform (тише, серый)
║                                                           ║
║                                                           ║
║  in room · 4:32 · ~1.2K tok · $0.003   ⏸ pause   tap stop║   ← бюджет + контролы
╚═══════════════════════════════════════════════════════════╝
```

Кто говорит — у того waveform активный (полный цвет), у второго — приглушён.
Subtitle её речи всплывает по мере TTS (по одному предложению).
Бюджет тикает в реальном времени.

### 16.9 Что НЕ делаем сейчас

Чтобы scope не разорвало:

- **Continuous voice без VAD** — нет, это слишком дорого и Соня не имеет continuous thinking (CRUTCH-002, до RWKV)
- **Multi-party voice** — нет, Atrium для двоих
- **Background voice listening вне комнаты** — нет, privacy + ресурсы
- **Voice memory отдельно от text memory** — нет, transcript идёт в episodic memory как обычные события
- **Голосовая клонировка Ивана** — нет, очевидно

### 16.10 Как это укладывается в Этапы

| Этап | Что |
|---|---|
| 0 | backend channels — `voice.speak`/`voice.listen` placeholders в OutgoingMessage; событие `dialog.interrupted` в substrate schema |
| 1 | Atrium UI без voice (room view как placeholder сцена с её SVG) |
| **2** | **Voice mode полноценный**: edge-tts streaming + whisper.cpp ASR + VAD + interrupt logic + room view с waveforms + tap-stop + budget counter |
| 3 | Live2D с lip-sync через viseme-mapping в voice mode |
| 4 | (post-RWKV) она может перебивать **тебя** через continuous thinking
...
- **2026-05-28 v4** — фидбек Ивана про voice + interrupt: (1) voice mode = войти в комнату, не toggle. Метафора пространства, не режим; (2) hold-to-talk микрофон в composer для quick voice messages вне комнаты; (3) interrupt четырьмя случаями: voice→voice (hard cut по VAD), text→voice (sentence boundary pause), voice→text (hold-to-talk не interrupt а input), tap-stop (физическое касание в комнате); (4) reply через voice = soft contextual binding, recent reason-stream events в её voice-step context; (5) hard binding через memory.recall если нужен конкретный event; (6) auto-leave через 5 минут тишины; (7) бюджет токенов видим снизу комнаты; (8) waveforms активного говорящего, subtitles её речи в реальном времени.
