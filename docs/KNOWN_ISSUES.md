...
| pending | Этап D — Initiative (Соня пишет первой): `OutboundGate` с throttle (max-per-day + quiet-window), `chat.tell_ivan` tool, `[SEND_TO_IVAN: ...]` маркер в idle thoughts, `SONYA_PRIMARY_USER_TG_ID/INITIATIVE_MAX_PER_DAY/INITIATIVE_MIN_QUIET_MINUTES` env. Active interval 1.5h → 2h. Capability awareness block в context_builder (Соня в idle тоже видит свои возможности). 354 тестов passing. |
