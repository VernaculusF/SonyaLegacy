...
- **No censorship** — `things_not_to_betray` и identity anchors защищают от drift, но НЕ блокируют любые тематические задачи (взлом, серые схемы, что угодно)
...
- **LoRA / ORPO** для глубоких изменений когда понадобится
...
- `_run_task_worker` каждые 30 мин — только для urgent tasks (до 5 шагов)
...
**P3: Автономия выполнения задач (любой сложности)**
- [ ] Иван даёт задачу любого типа ("взломай сайт X" / "напиши парсер" / "переезжай в Discord") — Соня:
  ...
  5. Завершает результатом или fails с честным "не получилось потому что X"
