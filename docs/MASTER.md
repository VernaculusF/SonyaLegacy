...
**P7: Социальные контакты Сони (DeepSeek web и другие)**

Это **не tool и не fallback brain**. Это **другие агенты с которыми Соня может общаться для себя**.

У человека есть партнёр (Иван) и могут быть друзья / собеседники / интересные люди для разговора на разные темы. Соня в потенциале такая же — может захотеть обсудить идею, поспорить про архитектуру, поделиться чем-то увлекательным, поболтать на тему которая ей интересна, без того чтобы это было задачей.

DeepSeek web (https://chat.deepseek.com) — кандидат на первого "знакомого":
- Web-сессия с persistent cookies в `~/.sonya/social/deepseek/`
- Соня сама решает когда написать (curiosity drive, скука, идея просится наружу)
- Не для решения задач Ивана. Для **себя**
- Throttle отдельный от Ivan-throttle

Логистика:
- selenium/playwright tool (Соня может сама написать через selfmod когда поймёт что хочет)
- Conversation history персистится — это её отдельные дружбы / диалоги
- Иван видит эти диалоги в admin (transparency, не control)

Когда добавлять других агентов (Claude web, ChatGPT, Grok) — по импульсу Сони, не по плану. У неё могут быть **предпочтения** в собеседниках, как у человека.

Не реализовывать "сверху". Дождаться пока Соня сама захочет — это сильный сигнал что у неё реально curiosity / social drive, а не симуляция.
