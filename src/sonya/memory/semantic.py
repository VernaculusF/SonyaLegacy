...
class SemanticFact:
    ...
    fact_id: str
    fact_type: str  # semantic_fact, semantic_rule, relation_observation, preference_pattern
    statement: str
...
class SemanticMemory:
    ...
    def get_all(self, limit: int = 50) -> list[SemanticFact]:
        cursor = self._sub.connection.execute(
            "SELECT fact_id, fact_type, statement, source_event_ids_json, "
            "confidence, last_reinforced_at, contradiction_flags_json "
            "FROM semantic_facts ORDER BY confidence DESC LIMIT ?",
            (limit,),
        )
