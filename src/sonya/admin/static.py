...
renderers.providers = function(d) {
  const s=d.settings||{}, ps=d.providers||[], as=d.accounts||[], ms=d.models||[], qs=d.quota_windows||[], os=d.observations||[], ks=d.keys||[], aos=d.account_offerings||[];
  providersSnapshot=d;
  ...
  const available=new Set((d.available_models||[]).map(x=>x.provider_model_key||`${x.provider}::${x.model_id}`)), q=providersModelQuery.toLowerCase();
  ...
  const modelKey=x=>x.provider_model_key||`${x.provider}::${x.model_id}`, freeOnlyProvider=p?.provider_id==='openrouter'||p?.provider_id==='nous';
  const pm=(freeOnlyProvider&&!q)?rawPm.filter(x=>x.is_free||available.has(modelKey(x))):rawPm;
  ...
  const modelsTitle=(freeOnlyProvider&&!q&&rawPm.length!==pm.length)?`Model pool · ${pm.length} free/requested of ${rawPm.length}`:`Model pool · ${pm.length}`;
  const models=p?`<div class="card"><div class="provider-head"><h3>${modelsTitle}</h3><input class="provider-filter" value="${escapeHtml(providersModelQuery)}" oninput="providersFilterModels(this)" placeholder="Filter models"></div><div class="provider-stack provider-models">${shown.map(m=>{const raw=m.metadata?.raw||{}, perAccount=aos.filter(o=>o.provider_id===m.provider&&o.model_id===m.model_id), limits=[raw.inputTokenLimit?`input ${Number(raw.inputTokenLimit).toLocaleString()}`:'',raw.outputTokenLimit?`output ${Number(raw.outputTokenLimit).toLocaleString()}`:''].filter(Boolean).join(' · ');return `<div class="provider-row"><div class="provider-head"><strong>${escapeHtml(m.model_name)}</strong><div>${m.is_free?badge('free'):''} ${badge(available.has(modelKey(m))?'available':'unavailable')} ${m.text_loop_ok?'':badge('special worker')}</div></div><div class="provider-meta">${escapeHtml(m.model_id)} · context ${Number(m.context_length||0).toLocaleString()} · ${(m.modalities||[]).map(escapeHtml).join(', ')||'text'} · ${escapeHtml(m.discovery_source||'manual')}${limits?' · '+escapeHtml(limits):''}</div>${perAccount.map(o=>`<div class="provider-meta">${escapeHtml(o.account_id)}: ${o.enabled?'enabled':'disabled'}${o.metadata?.disabled_reason?' · '+escapeHtml(o.metadata.disabled_reason):''}</div>`).join('')}<div class="provider-actions" style="margin-top:6px"><button class="provider-button" onclick="providersSetOffering('${m.model_id}',true)">Enable for account</button><button class="provider-button" onclick="providersSetOffering('${m.model_id}',false)">Disable for account</button></div></div>`}).join('')||'<div class="provider-meta">No matching models.</div>'}</div></div>`:'';
...
function providersFilterModels(input) { providersModelQuery=input.value; const pos=input.selectionStart; providersRerender(); const next=document.querySelector('.provider-filter'); if(next){next.focus();next.setSelectionRange(pos,pos);} }
